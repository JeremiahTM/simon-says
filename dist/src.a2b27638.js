...
var statusSpan = document.querySelector(".status"); // Use querySelector() to get the status element
var heading = document.querySelector("h1"); // Use querySelector() to get the heading element
var padContainer = document.querySelector(".pad-container"); // Use querySelector() to get the heading element
...
var maxRoundCount = setLevel(); // the max number of rounds, varies with the chosen level
...
function playComputerTurn() {
  ...
  var pad = getRandomItem(pads);
  computerSequence.push(pad);
...
function playHumanTurn() {
  ...
  setText(statusSpan, "Player's turn: ".concat(roundCount, " Presses left"));
}
...
function checkPress(color) {
  ...
  var index = playerSequence.lastIndexOf(color);
  ...
  if (playerSequence[index] !== computerSequence[index].color) {
    resetGame("Failed! You Chose the Wrong Color!");
  }
