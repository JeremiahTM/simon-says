...
let maxRoundCount = setLevel(); // the max number of rounds, varies with the chosen level
...
function activatePads(sequence) {
  ...
  sequence.forEach((element) => {
    delay = delay + 600
    setTimeout(() => {activatePad(element.color)},delay)
  })
}
...
function checkPress(color) {
  ...
  let index = playerSequence.lastIndexOf(color)
  ...
  if (playerSequence[index] !== computerSequence[index].color) {return resetGame("Failed! You Chose the Wrong Color!") }
...
function checkRound() {
  // TODO: Write your code here.
  if (playerSequence.length === maxRoundCount){resetGame("Congrats! You Win!")} else{
    roundCount += 1
    playerSequence =[]
...
//This is additional functions for music
